...
export function DailyGoal({ goal, progress, onGoal, reached }) {
  ...
  return (
    <div className={`daily-goal ${reached ? 'is-reached' : ''}`}>
      ...
      <div className="daily-goal__flames" aria-hidden>
        {flames.map((lit, i) => (
          <span key={i} className={`daily-goal__flame ${lit ? 'is-lit' : ''}`}>
            {lit ? '🔥' : '🪵'}
          </span>
        ))}
      </div>
